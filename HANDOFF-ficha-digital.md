...
**As decisões de desenho estão no `DESIGN-ficha-digital.md`**, e o resumo delas é:

| decisão | o que ficou |
|---|---|
| paleta | base `#211C35` e `#756588`, as cores do servidor, com os papéis corrigidos por contraste |
| automação | Apps Script, rodado uma vez por você na ficha-modelo |
| celular | uma quarta aba `MESA`, de 12 colunas, com o que muda durante a sessão |
| progressão | a ficha acompanha do nível 2 ao 30 e calcula sozinha; **o XP fica manual** |
| feitiços | linha de uso rápido na `MESA`, bloco completo na `TÉCNICA`, e a primeira puxa da segunda |
...
### O da maestria: uma frase ambígua que produziu número errado

A especificação dizia *"maestria: 1, e sobe +1 a cada oito níveis"*. A leitura natural disso é `1 + nível÷8`, e ela **erra nos níveis 8, 9, 16, 17, 24 e 25**: dá `+1` a mais na CD de feitiço e no ataque de conjuração.

A tabela da página 192 é a dona: maestria vira 2 no nível **10**, 3 no **18**, 4 no **26**. A fórmula certa é `1 + quantos de (10, 18, 26) ≤ nível`, e o `conferir-progressao.py` reproduz as trinta linhas.

**Já corrigido** na especificação, no catálogo e no protótipo. Fica registrado porque a frase ambígua pode voltar.
...
| `conferir-progressao.py` | as fórmulas contra a tabela do nível 1 ao 30 |
| `medidas/paleta.py` | deriva a paleta e mede o contraste |
| `medidas/daltonismo.py` | simula protanopia e deuteranopia sobre a paleta |
| `medidas/bloco-feitico.py` | prova que seis dos nove campos de feitiço são calculados |
| `DESIGN-ficha-digital.md` | paleta, tipografia, abas, reservas, progressão, automação |
| `PROMPT-ficha-digital.md` | o texto para colar na conversa nova |
| `PENDENCIAS.md` | **as nove decisões em aberto, com as opções e o preço de cada uma** |
...
1. **Resolva o bloco A do `PENDENCIAS.md`** antes de escrever qualquer código. São cinco decisões, e a primeira (*onde o catálogo mora*) piora sozinha conforme mais fichas entram em circulação.
