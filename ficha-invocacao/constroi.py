# -*- coding: utf-8 -*-
"""Constroi a ficha da invocacao DENTRO de um Workbook que o chamador abre.

Ela tem dois destinos, e por isso mora aqui em vez de num script:

  · ficha-invocacao/monta.py    arquivo solto, para mandar para alguem
  · ficha-v01/monta.py          aba da ficha do personagem

A diferenca entre os dois e uma so: soltas, as tres celulas do dono -- nivel,
...
def constroi(wb, dono=None, aba="INVOCAÇÃO", aba_catalogo="CATÁLOGO",
             aba_dados="DADOS"):
    """Escreve as tres abas da invocacao em `wb` e devolve o indice de campos.

    dono=None            -> as tres celulas do dono sao digitadas
    dono={"nivel":...}   -> elas viram formula. As chaves esperadas sao
                            `nivel`, `essencia` e `inteligencia`, cada uma com
                            a referencia completa, tipo "FICHA!$AH$11".
    """
    DONO, NI, NC, ND = dono, aba, aba_catalogo, aba_dados
    # ====================================================================== DADOS
    d = wb.create_sheet(ND)
    ...
    def lista(c, titulo, itens, larg=22):
        ...
        for i, v in enumerate(itens):
            txt(d, c, 2 + i, v)
        REF[titulo] = f"{ND}!${L(c)}$2:${L(c)}${1+len(itens)}"
    ...
    def tabela(c, titulo, cabec, linhas, larg=16):
        ...
        for i, lin in enumerate(linhas):
            for j, v in enumerate(lin):
                txt(d, c + j, 3 + i, v)
        REF[titulo] = f"{ND}!${L(c)}$3:${L(c+len(cabec)-1)}${2+len(linhas)}"
    ...
    # solta, as tres sao digitadas. Como aba da ficha grande, elas puxam da FICHA
    # e param de poder divergir dela.
    _v_niv = f'={DONO["nivel"]}' if DONO else 2
    _v_ess = f'={DONO["essencia"]}' if DONO else 0
    _v_int = f'={DONO["inteligencia"]}' if DONO else 0
    R["nivel"]    = G.campo(ws, GRADE_10[0], r, LARG_10, "nível do dono", _v_niv, pt=G.PT_GRANDE, alto=2)
    R["ess_dono"] = G.campo(ws, GRADE_10[1], r, LARG_10, "essência do dono", _v_ess, pt=G.PT_GRANDE, alto=2)
    R["int_dono"] = G.campo(ws, GRADE_10[2], r, LARG_10, "inteligência do dono", _v_int, pt=G.PT_GRANDE, alto=2)
    ...
    r = G.nota(ws, r + 3,
               "Estes três vêm da aba FICHA sozinhos. O resto desta seção a planilha calcula."
               if DONO else
               "Copie estes três da sua ficha. O resto desta seção a planilha calcula.")
    ...
    return {"campos": R, "slots": SLOTS, "nomes_slot": NOMES_SLOT,
            ...
            "n_comando": N_COMANDO, "atributos": ATR}
